...
// Initialize Firebase
    ...
    var chatDetails = {}
      ...
      function newMessage(key) {
          ...
          database.ref('candidate_students/'+"<?php echo $user_id; ?>"+'/chats/'+key+'/').push({
            type:'sent',
            message:message
          }).then(()=>{
            database.ref('delegate_students/'+key+'/chats/'+"<?php echo $user_id; ?>"+'/').push({
              type:'replies',
              message:message
            }).then(()=>{
        ...
        database.ref('delegate_students/').on('value',(snapShot)=>{
            if(snapShot.val()){
              ...
              Object.keys(delegatesObj).forEach((delegate_key)=>{
                if(delegate_key != 'counter'){
                  var currentDel = delegatesObj[delegate_key]
                  chatDetails[delegate_key] = currentDel
                  var contactItemOutbox = '<li class="contact" onclick="javascript:chatWith(this)">'+
                                        '<div class="wrap">'+
                                            '<img style="width:50px;height:50px;border-radius:50%;" src="'+currentDel.profile_photo+'" alt="" />'+
                                            '<div class="meta">'+
      ...
      function chatWith(elem){
          ...
          let delKey = $(elem).find('input').attr('value').toString().toUpperCase()
          ...
          var currentChat = '<img style="width:60px;height:60px;border-radius:50%;" id="chatting_contact_img" src="'+profPic+'" alt="" />'+
                            ...
                            '<input id="keyHolder" type="hidden" value="'+delKey+'"  />'

          $('#contact-profile').empty().append(currentChat)
          getChats(delKey,profPic)

      }
      function getChats(delegate_key,profPic){
        database.ref('candidate_students/'+"<?php echo $user_id; ?>"+'/chats/'+delegate_key+'/').on('value',(snapShot)=>{
            if(snapShot.val()){
              ...
              Object.keys(chatsObj).forEach((chat)=>{
                ...
                if(individualChat.type == 'sent'){
                  var chatItem = '<li class="sent">'+
                                    '<img style="width:30px;height:30px;" src="'+chatDetails["<?php echo $user_id; ?>"].toString()+'" alt="" />'+
                 ...
                 var chatItem = '<li class="replies">'+
                                   '<img style="float:left;width:30px;height:30px;" src="'+profPic+'" alt="" />'+
                                   '<p>'+individualChat.message+'</p>'+
                                '</li>'
                $('#messages_list').append(chatItem)
                var elem = document.getElementById('messages_list');
              ...
              $('#messages_list').empty()
              sendNoteSuccess('top','left','info','Currently no chats with this delegate. Initiate?', 5000)
            }
        })
      }
