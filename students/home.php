                    ...
                    <div class="carousel-inner" style="height:400px;width:100%">
                        <div class="item slides active" style="height:400px;width:100%">
                            ...
                            <div class="hero" style="z-index:20;">
                                ...
                                <button class="btn btn-hero btn-lg" role="button">Awesome! Vote Now</button>
                            </div>
                        </div>
...
<script type="text/javascript" >
    ...
    function updateFirebaseInitial(){
        ...
        databaseRef.ref('regular_students/'+user_id+'/').once('value',(snapshot)=>{
            if(!snapshot.val()){
                databaseRef.ref('regular_students/'+user_id+'/').update({
                    status:'online',
    ...
    databaseRef.ref('totals_delegates/').on('value',(snap)=>{
      if(snap.val()){
    ...
    databaseRef.ref('delegate_students/').on('value',(snapshot)=>{
      $('#slick-carousel').slick('unslick')
          if(snapshot.val()){
            var delegatesObj = snapshot.val()
            // console.log(delegatesObj)
              $('#slick-carousel').slick('unslick')
              Object.keys(delegatesObj).forEach((delegate)=>{
                    var individualDel = delegatesObj[delegate]
                    // console.log(individualDel)
                    var pic = individualDel.profile_photo
                    var name = individualDel.full_name
                    var votes = individualDel.votes.counter
                    var tots = document.getElementById('totals').value
                    // console.log(pic+name+votes+tots);
                    var percent = ((votes/tots) * 100).toFixed(2)
                    var slickElem = '<div class="twPc-div ">'+
                                        '<a class="twPc-bg twPc-block"></a>'+
                                        '<div>'+
                                            '<a title="delegate" href="delegates.php" class="twPc-avatarLink">'+
                                                '<img alt="delegate pic" src="'+individualDel.profile_photo+'" class="twPc-avatarImg">'+
                                            '</a>'+
                                            '<div class="twPc-divUser">'+
                                                '<div class="twPc-divName">'+
                                                    '<a href="delegates.php">'+individualDel.full_name+'</a>'+
                                                '</div>'+
                                            '</div>'+
                                            ...
                                            '<div class="twPc-divStats">'+
                                                '<ul class="twPc-Arrange">'+
                                                    '<li class="twPc-ArrangeSizeFit-votes">'+
                                                        '<a href="delegates.php" title="885 Following">'+
                                                            '<span class="twPc-StatLabel twPc-block">Votes</span>'+
                                                            '<span class="twPc-StatValue">🗳'+individualDel.votes.counter+'</span>'+
                                                        '</a>'+
                                                    '</li>'+
                                                    '<li class="twPc-ArrangeSizeFit-votes">'+
                                                        '<a href="delegates.php" title=" Following">'+
                                                            '<span class="twPc-StatLabel twPc-block">Likes</span>'+
                                                            '<span class="twPc-StatValue">👍'+individualDel.likes.counter+'</span>'+
                                                        '</a>'+
                                                    '</li>'+
                                                    '<li class="twPc-ArrangeSizeFit-percent">'+
                                                        '<a href="delegates.php" title=" Followers">'+
