...
<script type="text/javascript">
    ...
    function retrieveCandidates(){
      sendNoteSuccess('top','left','info','Fetching the candidates. Be Patient',1500)
    ...
    function voteCandidate(elem){
      ...
      sendNoteSuccess('top','left','info','Hey '+"<?php echo $user_name; ?>"+' Chill, submiting your vote',2000)
      databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/'+"<?php echo $user_id; ?>"+'/').once('value',(snapshot)=>{
        if(!snapshot.val()){
            var voting_time = new Date().toISOString()
            databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/'+"<?php echo $user_id; ?>"+'/').update({
              voter_name: "<?php echo $user_name; ?>",
              voting_time: voting_time
            }).then(()=>{
              databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/counter/').once('value',(snap)=>{

                  var current_prem_votes = snap.val() || 0
                  var new_prem_votes = parseInt(current_prem_votes) + 1
                  databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/').update({
                    counter: new_prem_votes
                  }).then(()=>{
                    databaseRef.ref('totals_candidates_preliminary/counter/').once('value',(snaps)=>{
                      var current_total_prem_votes = snaps.val() || 0
                      var new_total_prem_votes = parseInt(current_total_prem_votes) + 1
                      databaseRef.ref('totals_candidates_preliminary/').update({
                        counter: new_total_prem_votes
                      }).then(()=>{
                        sendNoteSuccess('top','left','success','Hey '+"<?php echo $user_name; ?>"+' Thank you for your vote, keep it coming',2000)
                        // retrieveCandidates()
                      })
                    })
                  }).catch((err)=>{
                    sendNoteError('top','right','danger','Preliminary votes update error: '+err.message,5000)
                  })

              }).catch((err)=>{
                sendNoteError('top','right','danger','Preliminary votes read error: '+err.message,5000)
              })
            }).catch((err)=>{
              sendNoteError('top','right','danger','Preliminary votes user add error: '+err.message,5000)
            })
        }else{
          var previous_voting_time = new Date(snapshot.val().voting_time).getTime()
          var time_now = new Date().getTime()
          var voting_time = new Date().toISOString()
          if(parseInt(time_now - previous_voting_time) < 86400000){
            sendNoteError('top','right','danger','Slow down there,You can only vote for a candidate every 24 hours in preliminary voting',5000)
          }else{
            databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/'+"<?php echo $user_id; ?>"+'/').update({
              voting_time: voting_time
            }).then(()=>{
              databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/counter/').once('value',(snap)=>{
                  ...
                  databaseRef.ref('candidate_students/'+candKey+'/preliminary_votes/').update({
                    ...
                    databaseRef.ref('totals_candidates_preliminary/counter/').once('value',(snap)=>{
                      var current_total_prem_votes = snap.val() || 0
                      var new_total_prem_votes = parseInt(current_total_prem_votes) + 1
                      databaseRef.ref('totals_candidates_preliminary/').update({
                        counter: new_total_prem_votes
                      }).then(()=>{
