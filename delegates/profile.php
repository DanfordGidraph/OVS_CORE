...
// Initialize Firebase
    ...
    $(document).ready(()=>{
      database.ref('delegate_students/'+"<?php echo $user_id; ?>"+'/about_delegate/').on('value',(snap)=>{
        if(snap.val()){
          var about = snap.val()
          $('#about_me').append(about)
        }
      })
      database.ref('delegate_students/'+"<?php echo $user_id; ?>"+'/delegate_skills/').on('value',(snap)=>{
        if(snap.val()){
          var skills = snap.val()
    ...
    function saveProfileEdits() {
      ...
      if(skills != '' && about != ''){
        database.ref('delegate_students/'+"<?php echo $user_id; ?>"+'/').update({
          delegate_skills: skills,
          about_delegate: about
        }).then(()=>{
